...
void Spider::Initialize(void)
{
	...
	BmpManager::GetInstance()->InsertBmp(L"../Image/370.bmp", L"370");
...
void Spider::LateUpdate(void)
{

}

void Spider::Render(HDC dc)
{
...
//	Rectangle(dc, m_rect.left + scroll_x, m_rect.top + scroll_y, m_rect.right + scroll_x, m_rect.bottom + scroll_y);
	...
	if (m_walk)
	{
		if (m_dir == DIR::UP)
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"370");
...
void Spider::SetCollision(Obj* obj, DIR::DIR dir_)
{
	if (m_dead)
		return;
