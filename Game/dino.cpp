...
void Dino::Initialize(void)
{
	m_id = OBJ::MONSTER;
	m_monster_id = MONSTER::DINO;
	...
	BmpManager::GetInstance()->InsertBmp(L"../Image/351_2.bmp", L"351_2");
	BmpManager::GetInstance()->InsertBmp(L"../Image/352_2.bmp", L"352_2");
	BmpManager::GetInstance()->InsertBmp(L"../Image/355.bmp", L"355");
...
void Dino::LateUpdate(void)
{

}

void Dino::Render(HDC dc)
{
	...
	if (m_walk)
	{
		...
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"351_2");
			if (m_walktime + 300.f < GetTickCount())
			{
				m_walktime = GetTickCount();
				m_walk = !m_walk;
	...
	else
	{
		...
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"352_2");
			if (m_walktime + 300.f < GetTickCount())
			{
				m_walktime = GetTickCount();
				m_walk = !m_walk;
	...
	if (m_dead)
	{
		mem_dc = BmpManager::GetInstance()->FindImage(L"355");
	}
...
void Dino::SetCollision(Obj* obj, DIR::DIR dir_)
{
	if (m_dead)
		return;
