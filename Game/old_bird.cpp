...
void OldBird::Initialize(void)
{
	m_id = OBJ::MONSTER;
	m_monster_id = MONSTER::OLD_BIRD;
	...
	BmpManager::GetInstance()->InsertBmp(L"../Image/404.bmp", L"404");
	BmpManager::GetInstance()->InsertBmp(L"../Image/405.bmp", L"405");
...
void OldBird::LateUpdate(void)
{

}

void OldBird::Render(HDC dc)
{
...
//	Rectangle(dc, m_rect.left + scroll_x, m_rect.top + scroll_y, m_rect.right + scroll_x, m_rect.bottom + scroll_y);
	...
	if (m_walk)
	{
		...
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"404");
	...
	else
	{
		...
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"405");
			if (m_walktime + 150.f < GetTickCount())
			{
				m_walktime = GetTickCount();
				m_walk = !m_walk;
...
void OldBird::Release(void)
{

}

void OldBird::SetVaild()
{
...
void OldBird::SetCollision(Obj* obj, DIR::DIR dir_)
{
	if (m_dead)
		return;
