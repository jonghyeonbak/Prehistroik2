...
void Bird::Initialize(void)
{
	...
	BmpManager::GetInstance()->InsertBmp(L"../Image/384.bmp", L"384");
	BmpManager::GetInstance()->InsertBmp(L"../Image/385.bmp", L"385");
	BmpManager::GetInstance()->InsertBmp(L"../Image/384_2.bmp", L"384_2");
	BmpManager::GetInstance()->InsertBmp(L"../Image/385_2.bmp", L"385_2");
...
void Bird::LateUpdate(void)
{

}

void Bird::Render(HDC dc)
{
...
//	Rectangle(dc, m_rect.left + scroll_x, m_rect.top + scroll_y, m_rect.right + scroll_x, m_rect.bottom + scroll_y);
	...
	if (m_walk)
	{
		if (m_dir == DIR::LEFT)
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"384_2");
		...
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"384");
			if (m_walktime + 150.f < GetTickCount())
			{
				m_walktime = GetTickCount();
				m_walk = !m_walk;
	...
	else
	{
		 if (m_dir == DIR::LEFT)
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"385_2");
			if (m_walktime + 150.f < GetTickCount())
			{
				m_walktime = GetTickCount();
				m_walk = !m_walk;
		...
		{
			mem_dc = BmpManager::GetInstance()->FindImage(L"385");
...
void Bird::SetCollision(Obj* obj, DIR::DIR dir_)
{
	if (m_dead)
		return;
